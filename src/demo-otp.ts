...
import { callAction, otpCommit, otpOperation, mockOtpEnabled, otpDigest, SessionError, twinRpc, type CallSession } from './call-session';
...
function displayCode(hash: string, challenge: string) {
  ...
  const bytes = createHmac('sha256', secret).update(`demo-display-v1:${hash}:${challenge}`).digest();
...
export async function readDemoOtp(hash: string, session: CallSession) {
  if (!mockOtpEnabled() || session.finalizedAt || session.verified || session.otpState !== 'pending'
    || !session.challengeId) return null;
  const code = displayCode(hash, session.challengeId);
  const prepared = await twinRpc('poc_call_action', { p_session_hash: hash, p_action: 'prepare_verify', p_challenge: session.challengeId });
  if (!prepared.ok || !prepared.verifier || !/^[a-f0-9]{64}$/.test(prepared.verifier)) return null;
  if (!timingSafeEqual(Buffer.from(otpDigest(hash, session.challengeId, code), 'hex'), Buffer.from(prepared.verifier, 'hex'))) return null;
...
export async function createOtpForCall(hash: string, operationId = randomUUID() as string, preparedChallenge?: string, deliver: () => Promise<boolean> = async () => true) {
  ...
  return otpOperation(hash, operationId, async () => {
    ...
    if (issue.replayed || issue.session?.otpState === 'pending') {
      if (issue.session?.otpState === 'pending' && !await readDemoOtp(hash, issue.session)) throw new SessionError('OTP_RESULT_UNCERTAIN');
      return issue;
    }
