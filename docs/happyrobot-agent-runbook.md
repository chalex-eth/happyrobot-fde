...
**Drizzle backend:** Drizzle schema definitions generate a schema-only migration (eight tables, zero SQL functions). TypeScript decisions and atomic SQL batches use the Twin SQL HTTP API with server-only `TWIN_API_KEY`. Local parity and an isolated live Drizzle test passed. Shared application tables have not been reset. The owner chose a fresh start: follow the [manual Twin setup](twin-fresh-start.md), including operator-key registration, before restarting this backend. See [architecture](architecture.md#database-generation) and [Drizzle validation](drizzle-backend-validation.md). `npm run db:parity` is local; `npm run db:verify-twin -- --allow-shared-scratch` explicitly creates/deletes isolated live scratch tables. Historical rollout links below refer to the legacy implementation.
...
| `apps/api/src/integrations/tms/inventory.ts`, `src/operator*.ts`, `app/api/operator/*`, `apps/api/db/tests/fixtures/legacy/twin-m5.sql` | Direct operator access, real TMS inventory, safe call projections, review queue and audit notes |
