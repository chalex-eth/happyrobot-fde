...
**Backend baseline branch:** Business decisions now execute in TypeScript; the new baseline is tested against the legacy SQL on disposable PostgreSQL. Shared Twin has not been upgraded. Do not restart/deploy this branch against the existing database without a separate schema/data rollout and backend credential provisioning. See [architecture](architecture.md#database-generation) and run `npm run db:parity`. Historical rollout and migration links below refer to the legacy implementation.
