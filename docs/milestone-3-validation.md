...
- Apply `docs/twin-m3.sql`, `docs/twin-m3.1.sql`, `docs/twin-m3.2.sql`, then `docs/twin-m3.5.sql` to a **fresh disposable PostgreSQL database**. Never rerun base migrations on the existing Twin workspace.
- In that disposable database run `tests/otp-transitions.sql`, `tests/call-transitions.sql`, `tests/finalize-transitions.sql`, `tests/negotiation-transitions.sql` with psql ON_ERROR_STOP=1. Fixtures roll back.
