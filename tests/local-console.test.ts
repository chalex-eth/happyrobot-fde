...
import { POST } from '../app/api/local/tms/route';
...
test('local console uses server credentials, rejects foreign origins, and is disabled in production', async () => {
  ...
  const request = (origin: string, host = '127.0.0.1:3000') => new Request('http://127.0.0.1:3000/api/local/tms', {
    method: 'POST', headers: { origin, host, 'content-type': 'application/json' }, body: '{}',
  });
  try {
    env.NODE_ENV = 'development'; env.LOCAL_API_TOKEN = 'server-only-test-token';
    ...
    for (const origin of ['', 'null', 'https://evil.example', 'http://127.0.0.1:4000', 'http://127.0.0.1:3000/']) {
