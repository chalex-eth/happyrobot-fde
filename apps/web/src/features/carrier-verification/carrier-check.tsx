...
import { LocalCallResponseSchema, type CallSession } from '@carrier/contracts/calls';
import { readApiResponse } from '../../lib/api-client';
import { VoiceCall } from '../voice-call/voice-call';

type View = {
  session: CallSession | null;
  demoOtp: { code: string; challengeId: string; failuresRemaining: number } | null;
};
...
export function CarrierVerification() {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      try {
        const response = await fetch('/api/local/calls', {
          method: 'POST',
        ...
        const result = await readApiResponse(response, LocalCallResponseSchema);
        if (controller.signal.aborted) return;
        setView({ session: result.session, demoOtp: result.demoOtp ?? null });
        setError('');
      } catch (error) {
        if (error instanceof Error && error.message === 'SESSION_REQUIRED') {
          if (!controller.signal.aborted) setView(empty);
          keepPolling = false;
  ...
  async function newCall() {
    ...
    try {
      ...
      setView({ session: result.session, demoOtp: null });
  ...
  const code =
    ...
    view.demoOtp?.challengeId === session.challengeId
      ? view.demoOtp.code
      : null;
  useEffect(() => {
    window.dispatchEvent(new Event('carrier-call-updated'));
    ...
    <div className="demo-banner">
      ...
      <div className="demo-intro">
        ...
        <div className="demo-cues">
          ...
          <span>
            From <strong>Salt Lake City</strong>
          </span>
          <span>
            Try a counter of <strong>$2,700</strong>
          </span>
        </div>
      </div>
      <div className="demo-interaction">
        ...
        <div className={'demo-code' + (code ? ' ready' : '')} aria-live="polite">
          {code ? (
            <>
              <span>Read this code to the agent</span>
              <output aria-label="Your verification code">{code}</output>
              <small>Demo code · displayed here instead of email/SMS</small>
            </>
          ) : (
            <>
              <span>Verification code</span>
              <p>Your code appears here during the call.</p>
              <small>Keep this page open and speak naturally.</small>
            </>
          )}
        </div>
        {error && (
