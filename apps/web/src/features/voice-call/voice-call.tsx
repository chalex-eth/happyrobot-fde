...
import { VoiceResponseSchema, type CallSession } from '@carrier/contracts/calls';
...
const errors: Record<string, string> = {
  ...
  HAPPYROBOT_VOICE_UNAVAILABLE:
    'HappyRobot could not start the call. Start a new call before trying again.',
...
export function VoiceCall({
  ...
  disabled: boolean;
  ensureCall: () => Promise<CallSession>;
  ...
  async function stopRun(callId: string) {
    const response = await fetch('/api/local/voice/end', {
      method: 'POST',
  ...
  async function start() {
    ...
    try {
      ...
      setPhase('starting');
      if (boundCall.current && session && Date.parse(session.expiresAt) > Date.now())
        await stopRun(boundCall.current);
      const call = await ensureCall();
      callId = call.callId;
      boundCall.current = callId;
      ...
      const result = await readApiResponse(response, VoiceResponseSchema).catch((error) => {
        throw new Error(
          errors[error instanceof Error ? error.message : ''] ??
            'Voice startup failed. Start a new call before retrying.',
        );
      });
      if (!mounted.current || boundCall.current !== callId || result.session.callId !== callId) {
        await stopRun(callId);
        return;
      }
      ...
      const connected = await voice.connect({
        onAgentConnected: () => {
          if (mounted.current) setAgentJoined(true);
        },
        onDisconnected: () => {
          if (boundCall.current !== callId) return;
          void fetch('/api/local/voice/disconnected', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ callId }),
            keepalive: true,
          ...
          if (mounted.current) {
            setPhase('ended');
            if (!starting.current) onActive(false);
      ...
      if (!mounted.current || boundCall.current !== callId) {
      ...
      setAgentJoined(connected.room.remoteParticipants.size > 0);
      setPhase('connected');
      try {
        await connected.room.startAudio();
      } catch {
        /* User can explicitly enable playback below. */
      }
      ...
      if (callId) {
        try {
          ...
          /* Audio is stopped locally; never retry startup automatically. */
        }
      }
      if (!mounted.current) return;
      setPhase('error');
        ...
        name === 'NotAllowedError'
          ...
          : name === 'NotFoundError'
            ...
            : name === 'NotReadableError'
              ? 'The microphone is busy or unavailable. Check your audio settings and try again.'
              : error instanceof Error && errorsHasMessage(error.message)
                ? error.message
  ...
  async function end() {
    ...
    try {
      ...
      setNotice(
        'The audio connection could not close cleanly. Close this page if audio continues.',
      );
    } finally {
      try {
        if (boundCall.current) await stopRun(boundCall.current);
      } catch {
        setNotice('Audio stopped. HappyRobot could not confirm the run ended.');
      }
      if (mounted.current) {
        setPhase('ended');
        onActive(false);
  ...
  return (
    <section className="voice-controls" aria-label="Demo call controls">
      <div className="connection">
        <button onClick={() => void start()} disabled={disabled || working || live}>
          {alreadyUsed ? 'Start another demo call' : 'Start demo call'}
        </button>
        {live && (
          <button
            className="secondary"
            ...
            onClick={async () => {
              ...
              setMuteBusy(true);
              try {
                if (muted) await current.unmute();
                else await current.mute();
                setMuted(current.isMuted());
              } catch {
                setNotice('The microphone could not be changed. Try again.');
              } finally {
                setMuteBusy(false);
              }
            }}
        ...
        {needsPlayback && live && (
          <button
            ...
            onClick={async () => {
              try {
                await connection.current?.room.startAudio();
                setNeedsPlayback(!connection.current?.room.canPlaybackAudio);
              } catch {
                setNotice('Allow audio playback in your browser and try again.');
              }
            }}
          >
            Enable audio
      ...
      <p role="status" aria-live="polite">
        {labels[phase]}
        {phase === 'connected' ? (agentJoined ? ' · Agent joined' : ' · Waiting for agent') : ''}
...
function errorsHasMessage(message: string) {
  return (
    ...
    message === 'Voice startup failed. Start a new call before retrying.'
  );
}
