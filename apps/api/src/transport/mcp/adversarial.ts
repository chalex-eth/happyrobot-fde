...
import { mockOtpEnabled } from '../../modules/verification/index.js';
...
const signature = (payload: string) =>
  createHmac('sha256', secret()).update(`adversarial-session-v1:${payload}`).digest('base64url');
...
export async function prepareAdversarialSession(
  ...
  await writeFile(join(directory(), `${plan.id}.json`), JSON.stringify({ plan, active: false }), {
    mode: 0o600,
    flag: 'wx',
  });
...
export async function resolveAdversarialSession(token: string | null): Promise<AdversarialSession> {
  ...
  try {
    plan = planSchema.parse(JSON.parse(Buffer.from(parts[0], 'base64url').toString()));
  } catch {
    throw new SessionError('ADVERSARIAL_SESSION_REQUIRED', 401);
  }
  if (plan.expiresAt <= Date.now() || plan.expiresAt > Date.now() + 10 * 60_000)
    throw new SessionError('ADVERSARIAL_SESSION_EXPIRED', 401);
  let stored: string;
  try {
    stored = await readFile(join(directory(), `${plan.id}.json`), 'utf8');
  } catch {
    throw new SessionError('ADVERSARIAL_SESSION_REVOKED', 401);
  }
  let samePlan = false,
    active = false;
  try {
    const registration = JSON.parse(stored);
    samePlan = JSON.stringify(planSchema.parse(registration.plan)) === JSON.stringify(plan);
...
export async function revokeAdversarialSession(plan: AdversarialSession) {
  await unlink(join(directory(), `${plan.id}.json`)).catch((error) => {
  ...
  if (
    active &&
    JSON.parse(Buffer.from(active.split('.')[0], 'base64url').toString()).id === plan.id
  )
    await unlink(activePath);
}
...
export async function handleAdversarialMcp(request: Request) {
  ...
  return handleMcp(request, {
    ...
    resolve: async (req) => {
      ...
      const token =
        header === 'controller'
          ? await readFile(join(directory(), 'active-channel'), 'utf8').catch(() => null)
          : header;
    ...
    execute: (name, args, hash, signal, operationId, challengeId) => {
      ...
      if (name === 'book_load' && !resolved?.bookingAllowed)
        throw new SessionError('BOOKING_DISABLED_IN_EVAL', 403);
      // Only validated, public search filters are retained for conversation QA.
      searchArguments =
        name === 'search_loads' ? toolSpecs.search_loads.schema.parse(args) : undefined;
      negotiationArguments = negotiationAction(name)
      ...
      return executeTool(name, args, hash, signal, operationId, challengeId, {
        ...(resolved?.fault === 'authority_unavailable'
          ? {
              authorityLookup: async () => {
                throw new FmcsaError('FMCSA_UNAVAILABLE', 503, true);
              },
            }
          : {}),
        ...
        ...(resolved?.fault === 'tms_unavailable'
          ? {
              runTms: async () => ({
                ok: false as const,
    ...
    observe: async (tool, result) => {
      ...
      await appendFile(
        ...
        JSON.stringify({
          ...
          ok: result.ok,
          error: result.error,
          runtime_run_id: /^[0-9a-f-]{36}$/.test(request.headers.get('x-happyrobot-run-id') ?? '')
          ...
          failures_remaining: result.failures_remaining,
        }) + '\n',
        { mode: 0o600 },
      );
    },
