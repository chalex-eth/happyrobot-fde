...
export function validateRequest(input: unknown): TmsRequest {
  if (!input || typeof input !== 'object' || Array.isArray(input))
    throw new TmsError('INVALID_REQUEST');
  const value = input as Record<string, unknown>;
  if (Object.keys(value).some((key) => !['command', 'fields'].includes(key)))
    throw new TmsError('INVALID_REQUEST');
  const command = value.command;
  if (typeof command !== 'string' || !['DEBUG_ECHO', 'LOAD_QUERY', 'LOAD_GET'].includes(command))
    throw new TmsError('INVALID_COMMAND');
  const fields = value.fields ?? {};
  if (!fields || typeof fields !== 'object' || Array.isArray(fields))
    throw new TmsError('INVALID_FIELDS');
  const allowed =
    command === 'LOAD_QUERY' ? queryFields : command === 'LOAD_GET' ? ['LOAD_ID'] : [];
  for (const [key, field] of Object.entries(fields)) {
    if (!allowed.includes(key) || !safeValue(field)) throw new TmsError('INVALID_FIELDS');
    if (key === 'MAX_RESULTS' && !/^(?:[1-9]|1[0-9]|20)$/.test(field))
      throw new TmsError('INVALID_FIELDS');
  ...
  if (command === 'LOAD_QUERY' && !Object.keys(entries).some((key) => key !== 'MAX_RESULTS'))
    throw new TmsError('FILTER_REQUIRED');
...
export function encodeRequest(input: TmsRequest, token: string): string {
  ...
  const fields =
    request.command === 'DEBUG_ECHO' ? { MSG: 'local-feasibility' } : (request.fields ?? {});
  const frame =
    [
      `CMD:${request.command}`,
      `AUTH:${token}`,
...
function parseFields(line: string): Record<string, string> {
  ...
  for (const pair of line.split('|')) {
    ...
    if (!/^[A-Z][A-Z0-9_]*$/.test(key) || key in fields)
      throw new TmsError('MALFORMED_RESPONSE', true);
...
export function parseResponse(lines: string[], request: TmsRequest): PublicLoad[] {
  if (request.command === 'DEBUG_ECHO') {
    if (lines.length !== 1 || !lines[0].startsWith('ECHO|'))
      throw new TmsError('MALFORMED_RESPONSE', true);
    const echo = parseFields(lines[0].slice(5));
    if (echo.AUTH !== 'OK' || echo.MSG !== 'local-feasibility')
      throw new TmsError('INVALID_ECHO', true);
    return [];
  }
  const records = lines.map((line) => {
    const fields = parseFields(line);
    if (requiredFields.some((key) => !fields[key])) throw new TmsError('MALFORMED_RESPONSE', true);
    if (!/^[A-Za-z0-9_-]{1,64}$/.test(fields.LOAD_ID))
      throw new TmsError('MALFORMED_RESPONSE', true);
    for (const key of ['RATE', 'MILES', 'WEIGHT', 'PIECES']) {
      if (fields[key] !== undefined && !/^\d+(?:\.\d+)?$/.test(fields[key]))
        throw new TmsError('MALFORMED_RESPONSE', true);
    }
    for (const key of ['PICKUP_DT', 'DELIVERY_DT']) {
      if (fields[key] !== undefined && !/^\d{14}$/.test(fields[key]))
        throw new TmsError('MALFORMED_RESPONSE', true);
    }
    // Private, unknown, and operator free-text fields never cross this diagnostic boundary.
    return Object.fromEntries(
      publicFields.filter((key) => key in fields).map((key) => [key, fields[key]]),
    ) as PublicLoad;
  });
  if (
    request.command === 'LOAD_GET' &&
    (records.length !== 1 || records[0].LOAD_ID !== request.fields?.LOAD_ID)
  )
    throw new TmsError('MALFORMED_RESPONSE', true);
  return records;
}
...
function sendOnce(
  ...
  return new Promise((resolve, reject) => {
    ...
    socket.on('data', (chunk: Buffer) => {
      ...
      if (chunk.some((byte) => byte > 127 || (byte < 32 && byte !== 10 && byte !== 13)))
        return finish(new TmsError('MALFORMED_RESPONSE', true));
      ...
      while ((boundary = pending.indexOf('\r\n')) !== -1) {
        ...
        if (line === 'END') {
          try {
            ...
            return finish(
              error instanceof TmsError ? error : new TmsError('MALFORMED_RESPONSE', true),
            );
          }
        }
        lines.push(line);
    ...
    function incomplete(event: 'end' | 'close') {
      ...
      console.warn(
        JSON.stringify({
          event: 'tms_incomplete_response',
          command: request.command,
          socket_event: event,
          bytes_received: bytes,
          complete_lines: lines.length,
          pending_bytes: Buffer.byteLength(pending, 'ascii'),
          pending_end_marker: pending === 'END',
        }),
      );
    ...
    try {
      ...
      if (!fault.retryable || attempt === 2 || signal?.aborted) {
        return {
          ...
          failures,
          error: fault.code,
          retryable: fault.retryable && !signal?.aborted,
        };
      }
      try {
...
export async function getLoadAvailability(loadId: string, signal?: AbortSignal) {
  ...
  const result = await executeTms(
    { command: 'LOAD_GET', fields: { LOAD_ID: loadId } },
    signal,
    (lines) => {
      if (lines.length === 1 && parseFields(lines[0]).STATUS === 'OPEN')
        pricing = parsePrivatePricing(lines);
    },
  );
