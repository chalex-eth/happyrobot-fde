...
const yesNo = (value: unknown): boolean | null =>
  value === 'Y' || value === true ? true : value === 'N' || value === false ? false : null;
...
export function parseCarrierCheck(payload: unknown, mcNumber: string): CarrierCheck {
  ...
  if (Array.isArray(content) && content.length === 0)
    return result('not_found', 'NO_CARRIER_FOUND');
  ...
  if (
    !['string', 'number'].includes(typeof c.dotNumber) ||
    !/^[1-9]\d{0,8}$/.test(String(c.dotNumber)) ||
    typeof c.legalName !== 'string' ||
    !c.legalName.trim() ||
    c.legalName.length > 300
  ) {
    throw new FmcsaError('FMCSA_INVALID_RESPONSE');
  }
  const allowed = yesNo(c.allowedToOperate ?? c.allowToOperate);
  const conflict =
    c.allowedToOperate !== undefined &&
    c.allowToOperate !== undefined &&
    yesNo(c.allowedToOperate) !== yesNo(c.allowToOperate);
  const carrier: NonNullable<CarrierCheck['carrier']> = {
    dotNumber: String(c.dotNumber),
    legalName: c.legalName.trim(),
    allowedToOperate: conflict ? null : allowed,
    outOfService: yesNo(c.outOfService),
    outOfServiceReported: c.outOfService !== undefined,
    commonAuthority: authority(c.commonAuthorityStatus),
    contractAuthority: authority(c.contractAuthorityStatus),
  };
  if (conflict || (carrier.allowedToOperate === true && carrier.outOfService === true))
    return result('unverified', 'CONFLICTING_AUTHORITY_DATA', carrier);
  if (carrier.allowedToOperate === false || carrier.outOfService === true)
    return result('ineligible', 'NOT_ALLOWED_TO_OPERATE', carrier);
  if (carrier.outOfServiceReported && carrier.outOfService === null)
    return result('unverified', 'INCOMPLETE_AUTHORITY_DATA', carrier);
  const active = carrier.commonAuthority === 'A' || carrier.contractAuthority === 'A';
  if (!active && carrier.commonAuthority !== null && carrier.contractAuthority !== null)
    return result('ineligible', 'NO_ACTIVE_CARRIER_AUTHORITY', carrier);
  ...
  if (carrier.allowedToOperate !== true || !active)
    return result('unverified', 'INCOMPLETE_AUTHORITY_DATA', carrier);
