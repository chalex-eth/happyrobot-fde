import type { BookingAction, RpcArgs, TwinResult } from '../../db/rpc-contracts/index.js';
...
export function publicBooking(value: Booking): Booking {
  if (
    !value ||
    !['pending', 'confirmed', 'rejected', 'uncertain'].includes(value.status) ||
    !value.attempt_id ||
    !value.load_id ||
    !Number.isFinite(value.agreed_rate) ||
    (value.status === 'confirmed' && (!value.reference || value.handoff_mock !== true))
  )
...
export type BookingRpc = (
  name: 'poc_book_call',
  args: RpcArgs<'poc_book_call'>,
) => Promise<TwinResult>;

export async function bookForCall(
  hash: string,
  args: { load_id: string; offer_id: string },
  signal?: AbortSignal,
  dependencies: { rpc: BookingRpc; pricing: typeof getLoadPricing; send: typeof bookTms } = {
    rpc: twinRpc,
  ...
  const rpc = (action: BookingAction, metadata: Record<string, unknown> = {}) =>
    dependencies.rpc('poc_book_call', {
      p_session_hash: hash,
      p_action: action,
      p_metadata: { loadId: args.load_id, offerId: args.offer_id, ...metadata },
    });
  ...
  const response = (booking: Booking) => ({
    ...
    booking_saved: booking.status === 'confirmed',
    booking_confirmed: booking.status === 'confirmed' && booking.simulated !== true,
  });
  if (checked.booking) return response(checked.booking);
  let detail: Awaited<ReturnType<typeof getLoadPricing>>;
  try {
    detail = await dependencies.pricing(args.load_id, signal);
  } catch (error) {
    // No booking has been claimed or sent. Save the failed preflight for operations.
    await rpc('preflight_failed', {
      error: error instanceof TmsError ? error.code : 'TMS_UNAVAILABLE',
    });
  ...
  const claimed = await rpc('claim', {
    attemptId,
    simulated,
    terms: detail.result.records[0],
    listedCents: detail.pricing.listedCents,
    maxCents: detail.pricing.maxCents,
  });
  ...
  if ((claimed.booking.simulated === true) !== simulated)
    throw new SessionError('BOOKING_MODE_MISMATCH', 409);
  ...
  try {
    ...
    result = simulated
      ? {
          status: 'confirmed',
          reference: `MOCK-${attemptId}`,
          timestamp: new Date().toISOString().replace(/\D/g, '').slice(0, 14),
        }
      : await dependencies.send(
          { loadId: args.load_id, mcNumber: claimed.mcNumber, agreedCents: claimed.agreedCents! },
          signal,
        );
  } catch {
    result = { status: 'uncertain', error: 'TMS_BOOKING_UNCERTAIN' };
  }
  try {
    const saved = await rpc('complete', { attemptId, result: { ...result, simulated } });
    if (saved.ok && saved.booking) return response(saved.booking);
  } catch {
    /* Read recovery only: never repeat the network mutation. */
  }
  try {
    const recovered = await rpc('status');
    if (recovered.ok && recovered.booking && recovered.booking.status !== 'pending')
      return response(recovered.booking);
  } catch {
    /* Persisted claim still prevents another send. */
  }
  ...
  return response({
    ...
    status: 'uncertain',
    handoff_mock: false,
    error: 'BOOKING_RECORD_UNCERTAIN',
  });
}
