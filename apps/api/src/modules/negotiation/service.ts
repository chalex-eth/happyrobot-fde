...
function decision(result: Awaited<ReturnType<typeof twinRpc>>) {
  if (!result.ok)
    throw new SessionError(result.error ?? 'NEGOTIATION_UNAVAILABLE', resultStatus(result));
  const n = result.negotiation;
  if (
    !n ||
    !['offered', 'agreed', 'rejected', 'failed'].includes(n.status) ||
    !Number.isInteger(n.counter_rounds) ||
    n.counter_rounds < 0 ||
    n.counter_rounds > 3 ||
    n.booking_confirmed !== false
  )
  ...
  return {
    ...
    agreed_rate: n.agreed_rate,
    counter_rounds: n.counter_rounds,
    rounds_remaining: n.rounds_remaining,
    expires_at: n.expires_at,
    booking_confirmed: false as const,
  };
}

export async function getNegotiableLoad(
  hash: string,
  loadId: string,
  signal?: AbortSignal,
  ...
  const result = await loadsForCall(
    hash,
    { command: 'LOAD_GET', fields: { LOAD_ID: loadId } },
    signal,
    async () => {
      const detail = await pricingLookup(loadId, signal);
      prices = detail.pricing;
  ...
  if (status !== 'OPEN')
    return {
      ...result,
      negotiation: null,
      availability: status === 'PENDING' ? 'pending' : 'unavailable',
      can_negotiate: false,
      can_book: false,
      manager_review_available: status === 'PENDING',
    };
  if (!prices) throw new SessionError('TMS_PRICING_UNAVAILABLE');
  const quoted = await twinRpc('poc_negotiate', {
    p_session_hash: hash,
    p_action: 'quote',
    p_load_id: loadId,
    p_revision: before.session!.authorityRevision,
    p_listed_cents: prices.listedCents,
    p_max_cents: prices.maxCents,
  });
...
export async function negotiateForCall(
  hash: string,
  args: {
    load_id: string;
    offer_id: string;
  ...
  const result = await twinRpc('poc_negotiate', {
    p_session_hash: hash,
    p_action: args.response,
    p_load_id: args.load_id,
    p_offer_id: args.offer_id,
