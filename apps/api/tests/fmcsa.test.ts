...
before(() => {
  process.env.FMCSA_API_KEY = 'fixture-secret';
});
after(() => {
  if (previous === undefined) delete process.env.FMCSA_API_KEY;
  else process.env.FMCSA_API_KEY = previous;
});
const jsonFetch =
...
test('normalizes MC formats and rejects arbitrary URLs and malformed input', () => {
  ...
  for (const mc of ['', '0', 'MC123abc', '12/34', 'https://evil.example', 1515, '123456789'])
    assert.throws(() => normalizeMc(mc));
});
test('explicit active common or contract authority passes with operation and OOS flags', () => {
  ...
  assert.equal(
    parseCarrierCheck(payload({ commonAuthorityStatus: 'N', contractAuthorityStatus: 'A' }), '1515')
      .eligible,
    true,
  );
  assert.equal(
    parseCarrierCheck(
      { content: { carrier: { ...carrier, allowedToOperate: undefined, allowToOperate: 'Y' } } },
      '1515',
    ).eligible,
    true,
  );
...
test('inactive, out-of-service and broker-only carriers cannot pass', () => {
  for (const fields of [
    { allowedToOperate: 'N' },
    { allowedToOperate: 'N', outOfService: 'Y' },
    { commonAuthorityStatus: 'I' },
    { commonAuthorityStatus: 'N', brokerAuthorityStatus: 'A' },
  ]) {
...
test('the observed live response passes without an optional out-of-service field', () => {
    ...
    content: [
      {
        carrier: {
          dotNumber: 1078021,
          legalName: 'LESTER MOVING & STORAGE COMPANY',
          allowedToOperate: 'Y',
          commonAuthorityStatus: 'A',
          contractAuthorityStatus: 'N',
          oosDate: null,
  ...
  assert.equal(
    parseCarrierCheck(payload({ outOfService: undefined, allowedToOperate: undefined }), '1515')
      .outcome,
    'unverified',
  );
  assert.equal(
    parseCarrierCheck(payload({ outOfService: undefined, commonAuthorityStatus: 'N' }), '1515')
      .outcome,
    'ineligible',
  );
});
...
test('distinguishes confirmed empty data, ambiguous carriers and malformed responses', () => {
  ...
  assert.equal(
    parseCarrierCheck({ content: [{ carrier }, { carrier }] }, '1515').outcome,
    'unverified',
  );
  for (const value of [
    null,
    ...
    assert.throws(() => parseCarrierCheck(value, '1515'));
});
test('returns only safe carrier fields', () => {
  const result = parseCarrierCheck(
    payload({ phone: 'private', ein: 'private', links: [{ href: 'secret-url' }] }),
    '1515',
  );
...
test('handles auth denial, throttling, upstream failure and invalid JSON without leaking details', async () => {
  for (const [status, code] of [
    [401, 'FMCSA_ACCESS_DENIED'],
    [403, 'FMCSA_ACCESS_DENIED'],
    [429, 'FMCSA_RATE_LIMITED'],
    [500, 'FMCSA_UNAVAILABLE'],
    [404, 'FMCSA_UNAVAILABLE'],
  ] as const) {
    await assert.rejects(
      lookupCarrier('1515', undefined, async () => new Response('fixture-secret', { status })),
      (e: unknown) =>
        e instanceof FmcsaError && e.code === code && !e.message.includes('fixture-secret'),
    );
  }
  await assert.rejects(
    lookupCarrier('1515', undefined, async () => new Response('<html>bad</html>')),
    { code: 'FMCSA_INVALID_RESPONSE' },
  );
  await assert.rejects(
    lookupCarrier('1515', undefined, async () => {
      throw new Error('https://upstream?webKey=fixture-secret');
    }),
...
test('rejects oversized responses', async () => {
  await assert.rejects(
    lookupCarrier('1515', undefined, async () => new Response('x'.repeat(256 * 1024 + 1))),
    { code: 'FMCSA_INVALID_RESPONSE' },
  );
});
test('propagates cancellation and classifies deadline errors', async () => {
  ...
  controller.abort();
  await assert.rejects(
    lookupCarrier('1515', controller.signal, async (_url, init) => {
