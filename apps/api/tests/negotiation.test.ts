...
import {
  ...
  parseResponse,
  getLoadPricing,
} from '../src/integrations/tms/client.js';
import { getNegotiableLoad } from '../src/modules/negotiation/index.js';
import { executeTool } from '../src/transport/mcp/tools.js';
import { toolParameters } from '../../../scripts/happyrobot/workflow-spec.js';
const line =
  'LOAD_ID:L1|ORIG_CITY:A|ORIG_STATE:TX|ORIG_ZIP:75001|DEST_CITY:B|DEST_STATE:CA|DEST_ZIP:90001|PICKUP_DT:20260910080000|EQTYPE:FLATBED|RATE:1000|MILES:900|STATUS:OPEN|MAX_BUY:1200|NOTES:private';

test('private pricing is validated as cents while public detail excludes ceilings and notes', () => {
  assert.equal(moneyCents('0.01'), 1);
  assert.equal(moneyCents('1020.10'), 102010);
  for (const s of ['0', '-1', '1.001', 'NaN', 'Infinity', '1e3', '1000001'])
    assert.throws(() => moneyCents(s));
  assert.deepEqual(parsePrivatePricing([line]), { listedCents: 100000, maxCents: 120000 });
  for (const s of [
    line.replace('|MAX_BUY:1200', ''),
    line.replace('MAX_BUY:1200', 'MAX_BUY:999'),
    line.replace('STATUS:OPEN', 'STATUS:BOOKED'),
  ])
    assert.throws(() => parsePrivatePricing([s]));
  const result = parseResponse([line], { command: 'LOAD_GET', fields: { LOAD_ID: 'L1' } });
  assert.ok(!JSON.stringify(result).includes('MAX_BUY'));
  assert.ok(!JSON.stringify(result).includes('1200'));
  assert.ok(!JSON.stringify(result).includes('private'));
});

test('private pricing requires a complete TMS response and retries an incomplete frame', async (t) => {
  const previous = Object.fromEntries(
    ['TMS_HOST', 'TMS_PORT', 'TMS_TOKEN'].map((k) => [k, process.env[k]]),
  );
  let calls = 0;
  ...
  Object.assign(process.env, {
    TMS_HOST: '127.0.0.1',
    TMS_PORT: String((server.address() as net.AddressInfo).port),
    TMS_TOKEN: 'test',
  });
  t.after(async () => {
  ...
  const r = await getLoadPricing('L1');
  assert.equal(calls, 2);
  assert.equal(r.result.attempts, 2);
  assert.deepEqual(r.pricing, { listedCents: 100000, maxCents: 120000 });
  assert.ok(!JSON.stringify(r.result).includes('MAX_BUY'));
});
...
test('selected detail stores pricing only in the negotiation RPC, exposes the public offer and preserves authority revision', async (t) => {
  ...
  mockCommandsAndFetch(t, async (url: URL, init: RequestInit) => {
    ...
    if (String(url).endsWith('poc_negotiate')) {
      assert.equal(b.p_revision, 7);
      assert.equal(b.p_max_cents, 120000);
      assert.equal(b.p_listed_cents, 100000);
  ...
  const lookup = async () => {
    ...
    return {
      result: {
        ok: true as const,
        command: 'LOAD_GET' as const,
        complete: true,
        elapsed_ms: 1,
        attempts: 1,
        failures: [],
        record_count: 1,
        records: parseResponse([line], { command: 'LOAD_GET', fields: { LOAD_ID: 'L1' } }),
      },
      pricing: { listedCents: 100000, maxCents: 120000 },
    };
  };
  ...
  assert.ok(!JSON.stringify(r).includes('120000'));
  assert.ok(!JSON.stringify(r).includes('max_cents'));
  gate = false;
  events.length = 0;
  await assert.rejects(
    () => getNegotiableLoad('a'.repeat(64), 'L1', undefined, lookup),
    /OTP_REQUIRED/,
  );
  assert.ok(!events.includes('tcp'));
});
...
test('negotiation stays unavailable until its migration-backed feature is activated', async (t) => {
  ...
  await assert.rejects(
    () =>
      executeTool(
        'accept_offer',
        { load_id: 'L1', offer_id: '11111111-1111-4111-8111-111111111111' },
        'a'.repeat(64),
      ),
    /NEGOTIATION_NOT_READY/,
  );
});
...
test('counter then acceptance explicitly clears the previous amount without relaxing numeric acceptance validation', async (t) => {
  ...
  mockCommandsAndFetch(t, async (_url: unknown, init: RequestInit) => {
    ...
    return Response.json({
      ok: true,
      negotiation: {
        status: b.p_action === 'accept' ? 'agreed' : 'offered',
        load_id: 'L1',
        offer_id: next,
        offered_rate: 3496.56,
        agreed_rate: b.p_action === 'accept' ? 3496.56 : null,
        counter_rounds: 1,
        rounds_remaining: 2,
        booking_confirmed: false,
      },
  ...
  await executeTool(
    'counter_offer',
    { load_id: 'L1', offer_id: first, amount: '6856' },
    'a'.repeat(64),
  );
  const accepted = await executeTool(
    'accept_offer',
    { load_id: 'L1', offer_id: next },
    'a'.repeat(64),
  );
  assert.equal((accepted.negotiation as { status: string }).status, 'agreed');
  assert.deepEqual(
    requests.map((r) => [r.p_action, r.p_offer_id, r.p_amount_cents]),
    [
      ['counter', first, 685600],
      ['accept', next, null],
    ],
  );
  for (const response of ['accept', 'reject'])
    await assert.rejects(() =>
      executeTool(
        response === 'accept' ? 'accept_offer' : 'reject_offer',
        { load_id: 'L1', offer_id: next, amount: 6856 },
        'a'.repeat(64),
      ),
    );
  await assert.rejects(() =>
    executeTool('counter_offer', { load_id: 'L1', offer_id: next, amount: null }, 'a'.repeat(64)),
  );
