...
import { handleTmsRequest } from '../src/transport/http/tms.js';
import { parseResponse, runTms } from '../src/integrations/tms/client.js';

const previous = process.env.LOCAL_API_TOKEN;
before(() => {
  process.env.LOCAL_API_TOKEN = 'test-only';
});
after(() => {
  if (previous === undefined) delete process.env.LOCAL_API_TOKEN;
  else process.env.LOCAL_API_TOKEN = previous;
});
const request = (body: string, token = 'test-only', mime = 'application/json') =>
  new Request('http://localhost/api/tms', {
    method: 'POST',
    headers: {
...
const forbidden: typeof runTms = async () => {
  throw new Error('Transport must not run');
};

test('authentication rejects wrong or missing bearer before transport', async () => {
  for (const token of ['', 'wrong'])
    assert.equal((await handleTmsRequest(request('{}', token), forbidden)).status, 401);
});
test('missing app token fails closed', async () => {
  delete process.env.LOCAL_API_TOKEN;
  try {
    assert.equal((await handleTmsRequest(request('{}'), forbidden)).status, 503);
  } finally {
    process.env.LOCAL_API_TOKEN = 'test-only';
  }
});
test('rejects malformed JSON, incorrect MIME and oversized requests', async () => {
  ...
  assert.equal(
    (await handleTmsRequest(request('{}', 'test-only', 'application/jsonp'), forbidden)).status,
    415,
  );
...
test('rejects booking, missing filters and command injection', async () => {
  for (const body of [
    { command: 'LOAD_BOOK' },
    { command: 'LOAD_QUERY' },
    { command: 'LOAD_GET', fields: { LOAD_ID: 'X|CMD:LOAD_BOOK' } },
  ]) {
