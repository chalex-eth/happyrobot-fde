...
import { verifyCarrierForCall, verifyOtpForCall } from '../src/modules/verification/index.js';
import { createVoiceSession } from '../src/modules/calls/index.js';
import { resolveAgentSession } from '../src/transport/mcp/session.js';
import { FmcsaError } from '../src/integrations/fmcsa/client.js';
import { otpDigest } from '../src/modules/verification/index.js';
...
test('authority lookup runs only after call invalidation and uses the same identity and revision', async (t) => {
  ...
  t.mock.method(globalThis, 'fetch', async (_url: URL, init: RequestInit) => {
    ...
    if (b.p_action === 'authority_begin') invalidated = true;
      ...
      assert.equal(b.p_metadata.revision, 1);
      assert.equal(b.p_metadata.check.mcNumber, '1515');
    }
    return Response.json({ ok: true, session });
  });
  assert.equal(
    (
      await verifyCarrierForCall(hash, 'MC-1515', undefined, async () => {
...
test('failed FMCSA recheck records unverified evidence without copying upstream diagnostics', async (t) => {
  ...
  await assert.rejects(
    () =>
      verifyCarrierForCall(hash, '1515', undefined, async () => {
        throw new FmcsaError('FMCSA_ACCESS_DENIED', 503);
      }),
    FmcsaError,
  );
  const final = JSON.parse(writes[1]);
  assert.equal(final.p_metadata.check.eligible, false);
  assert.equal(final.p_metadata.check.reason, 'AUTHORITY_LOOKUP_FAILED');
});
...
test('shared load service refuses diagnostics and denied sessions before TCP; suppresses an in-flight result after carrier change', async (t) => {
  ...
  await assert.rejects(
    ...
    (e) => e instanceof SessionError && e.code === 'CALL_CHANGED',
  );
...
test('agent resolver rejects missing auth, invented IDs and unbound runs before returning internal state', async (t) => {
  ...
  for (const [auth, run] of [
    [null, id],
    ['Bearer wrong', id],
    ['Bearer test-only', '1515'],
    ['Bearer test-only', null],
  ]) {
    await assert.rejects(
      () => resolveAgentSession(auth, run),
      (e) => e instanceof SessionError && e.status === 401,
    );
  }
...
test('browser and agent verification use the bound call challenge, and another call cannot reuse its OTP digest', async (t) => {
  ...
  t.mock.method(globalThis, 'fetch', async (_url: URL, init: RequestInit) => {
    ...
    if (b.p_action === 'prepare_verify')
      return Response.json({ ok: true, verifier: otpDigest(hash, id, '001234') });
    matched.push(b.p_matches);
    return Response.json({ ok: b.p_matches, error: b.p_matches ? null : 'OTP_INVALID', session });
  });
...
test('voice startup binds the provider run before returning its token and never lets duplicate reservation create another run', async (t) => {
  ...
  t.mock.method(globalThis, 'fetch', async (_url: URL, init: RequestInit) => {
    ...
    if (b.p_action === 'voice_bind') {
      assert.equal(b.p_metadata.runId, id);
      bound = true;
    }
    return Response.json({ ok: true, session });
  });
  const result = await createVoiceSession(hash, sdk as never);
  assert.ok(bound);
  assert.equal(result.voice.run_id, id);
  await assert.rejects(
    () => createVoiceSession(hash, sdk as never),
    (e) => e instanceof SessionError && e.code === 'VOICE_ALREADY_STARTED',
  );
...
test('ending voice resolves the run from the authenticated call and rejects a different call ID', async (t) => {
  configure(t);
  const { endVoiceSession } = await import('../src/modules/calls/index.js');
  const runId = '22222222-2222-4222-8222-222222222222';
  const cancellations: string[] = [];
  const sdk = {
    runs: {
      cancel: async (run: string) => {
        cancellations.push(run);
      },
    },
  };
  t.mock.method(globalThis, 'fetch', async () =>
  ...
  await assert.rejects(
    () => endVoiceSession(hash, runId, sdk as never),
    (e) => e instanceof SessionError && e.code === 'CALL_CHANGED',
  );
...
test('voice HTTP routes reject absent cookies, foreign origins and caller-selected run IDs', async (t) => {
  ...
  const { POST: start } = await import('../src/transport/http/routes/local/voice/route.js');
  const { POST: end } = await import('../src/transport/http/routes/local/voice/end/route.js');
  const req = (body: object, origin = 'http://127.0.0.1:3000') =>
    new Request('http://127.0.0.1:3000/api/local/voice', {
      method: 'POST',
