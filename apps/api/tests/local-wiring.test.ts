...
import { toolSpecs, type ToolName } from '../src/transport/mcp/tools.js';
import { toolParameters, variable } from '../../../scripts/happyrobot/workflow-spec.js';
import { validateLocalWiring, type WiringNode } from '../../../scripts/happyrobot/local-wiring.js';

function fixture(): WiringNode[] {
  return [
    { id: 'prompt', type: 'prompt', name: 'Carrier sales conversation' },
    ...Object.keys(toolSpecs).flatMap((name, index) => {
      const id = `forked-${index}`,
        stable = `stable-${index}`;
      return [
        {
          id,
          persistent_id: stable,
          type: 'tool',
          parent_id: 'prompt',
          name,
          function: {
            ...
            is_mcp: true,
            mcp_tool_name: name,
            mcp_server_credential_id: 'connection',
          },
        },
        {
          id: `action-${index}`,
          ...
          configuration: {
            ...
            credential: { type: 'static', static: { id: 'connection' } },
            dynamic_headers: [{ key: 'x-happyrobot-run-id', value: variable('current', 'run_id') }],
            tool_args: toolParameters(name as ToolName).map((p) => ({
              key: p.name,
              value: variable(stable, p.name),
            })),
          },
        },
      ];
...
test('startup accepts normal wiring and rejects the isolated eval header', () => {
  ...
  nodes.find((n) => n.type === 'action')!.configuration!.dynamic_headers = [
    { key: 'x-adversarial-session', value: 'controller' },
  ];
