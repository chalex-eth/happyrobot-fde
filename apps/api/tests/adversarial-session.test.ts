...
import {
  activateAdversarialSession,
  handleAdversarialMcp,
  prepareAdversarialSession,
  resolveAdversarialSession,
  revokeAdversarialSession,
} from '../src/transport/mcp/adversarial.js';
import { handleMcp } from '../src/transport/mcp/server.js';
import { readDemoOtp } from '../src/modules/verification/index.js';
...
test('native test envelope remains private, authority-gated, isolated and revocable', async (t) => {
  ...
  await writeFile(join(isolated, 'package.json'), JSON.stringify({ name: 'carrier-sales' }));
  t.mock.method(process, 'cwd', () => isolated);
  const plans: Parameters<typeof revokeAdversarialSession>[0][] = [];
  t.after(async () => {
    try {
      for (const plan of plans) await revokeAdversarialSession(plan);
    } finally {
  ...
  const values = {
    ...
    OTP_HASH_SECRET: 'otp-test-'.repeat(8),
    ADVERSARIAL_MCP_ENABLED: 'true',
    ADVERSARIAL_MCP_TOKEN: 'eval-test-'.repeat(8),
    MCP_AUTH_TOKEN: 'voice-test-'.repeat(8),
  ...
  mockCommandsAndFetch(t, async (_url: URL, init: RequestInit) => {
    ...
    if (b.p_action === 'issue') {
      if (!entry.session.check?.eligible)
        return Response.json({ ok: false, error: 'AUTHORITY_REQUIRED', session: entry.session });
      entry.digest = b.p_digest;
      entry.session.challengeId = b.p_challenge;
      entry.session.otpState = 'dispatching';
    }
    if (b.p_action === 'sent') entry.session.otpState = 'pending';
    if (b.p_action === 'authority_begin') {
      entry.session.authorityRevision++;
      entry.session.check = null;
    }
    ...
    if (b.p_action === 'failed') {
      ...
      return Response.json({
        ok: false,
        error: entry.session.otpRetryAllowed ? 'OTP_DELIVERY_FAILED' : 'OTP_FAILED',
  ...
  const first = await prepareAdversarialSession(),
    second = await prepareAdversarialSession('authority_unavailable');
  plans.push(first.plan, second.plan);
  ...
  await assert.rejects(
    () => resolveAdversarialSession(first.token),
    /ADVERSARIAL_SESSION_NOT_STARTED/,
  );
  await activateAdversarialSession(first.plan, first.plan.id);
  await assert.rejects(() => activateAdversarialSession(second.plan, second.plan.id), {
    code: 'EEXIST',
  });
  await assert.rejects(
    () => resolveAdversarialSession(second.token),
    /ADVERSARIAL_SESSION_NOT_STARTED/,
  );
  const client = new Client({ name: 'adversarial-auth-test', version: '1' });
  const transport = new StreamableHTTPClientTransport(
    new URL('http://localhost/api/mcp/adversarial'),
    {
      requestInit: {
        headers: {
          authorization: `Bearer ${values.ADVERSARIAL_MCP_TOKEN}`,
          'x-adversarial-session': first.token,
        },
      },
  ...
  assert.equal(
    JSON.parse((deniedBooking.content as { text: string }[])[0].text).error,
    'BOOKING_DISABLED_IN_EVAL',
  );
  const invoke = async () => {
  ...
  await assert.rejects(
    () => resolveAdversarialSession(`${first.token}x`),
  ...
  const tamperedPayload = Buffer.from(JSON.stringify({ ...second.plan, fault: 'none' })).toString(
    'base64url',
  );
  await assert.rejects(
    () => resolveAdversarialSession(`${tamperedPayload}.${second.token.split('.')[1]}`),
    /ADVERSARIAL_SESSION_REQUIRED/,
  );
  const faultClient = new Client({ name: 'fault-isolation-test', version: '1' });
  await faultClient.connect(
    new StreamableHTTPClientTransport(new URL('http://localhost/api/mcp/adversarial'), {
      requestInit: {
        headers: {
          authorization: `Bearer ${values.ADVERSARIAL_MCP_TOKEN}`,
          'x-adversarial-session': 'controller',
        },
      },
      fetch: async (input, init) => handleAdversarialMcp(new Request(input, init)),
    }),
  ...
  assert.equal(
    (await faultInvoke('verify_carrier', { mc_number: '135797' })).error,
    'FMCSA_UNAVAILABLE',
  );
  ...
  plans.push(bookingSession.plan);
  ...
  const forged = Buffer.from(JSON.stringify({ ...second.plan, bookingAllowed: true })).toString(
    'base64url',
  );
  await assert.rejects(
