-- M4.2: isolate simulated bookings from real inventory claims; preserve all history.
-- Apply once after M4.1. Simulation is selected server-side, never by tool arguments.
BEGIN;
DROP INDEX public.poc_booking_load_claim;
CREATE UNIQUE INDEX poc_booking_load_claim ON public.poc_calls ((booking->>'load_id'))
 WHERE booking->>'status' IN ('pending','confirmed','uncertain') AND coalesce(booking->>'simulated','false')<>'true';

CREATE OR REPLACE FUNCTION public.poc_book_call(p_session_hash text,p_action text,p_metadata jsonb DEFAULT '{}'::jsonb)
...
BEGIN
 ...
 IF p_action='complete' THEN
  ...
  IF c.booking->>'status'='uncertain' AND state='uncertain' AND c.booking->>'error'=result->>'error' THEN
   RETURN jsonb_build_object('ok',true,'booking',c.booking);
  END IF;
  IF coalesce((result->>'simulated')::boolean,false) IS DISTINCT FROM coalesce((c.booking->>'simulated')::boolean,false) THEN
   RETURN jsonb_build_object('ok',false,'error','BOOKING_MODE_MISMATCH');
  END IF;
  ...
  IF state='confirmed' THEN
   b:=b || jsonb_build_object('reference',result->>'reference',CASE WHEN b->>'simulated'='true' THEN 'simulated_at_utc' ELSE 'tms_timestamp_utc' END,result->>'timestamp');
  ELSE
  ...
  IF c.finalized_at IS NOT NULL THEN
   outcome:=CASE state WHEN 'confirmed' THEN CASE WHEN b->>'simulated'='true' THEN 'booking_simulated' ELSE 'booked' END WHEN 'rejected' THEN 'booking_failed' ELSE 'booking_uncertain' END;
 ...
 IF NOT coalesce((p_metadata->>'simulated')::boolean,false) AND EXISTS(SELECT 1 FROM public.poc_calls WHERE booking->>'load_id'=n.load_id AND booking->>'status' IN ('pending','confirmed','uncertain') AND coalesce(booking->>'simulated','false')<>'true') THEN
 ...
 b:=jsonb_build_object('status','pending','attempt_id',p_metadata->>'attemptId','load_id',n.load_id,
  'offer_id',n.offer_id,'mc_number',mc,'agreed_rate',n.agreed_cents/100.0,'attempted_at',now(),'handoff_mock',false,'simulated',coalesce((p_metadata->>'simulated')::boolean,false));
 UPDATE public.poc_calls SET booking=b WHERE id=c.id;
...
BEGIN
 ...
 IF b IS NOT NULL THEN
  p_outcome:=CASE b->>'status' WHEN 'confirmed' THEN CASE WHEN b->>'simulated'='true' THEN 'booking_simulated' ELSE 'booked' END WHEN 'rejected' THEN 'booking_failed' ELSE 'booking_uncertain' END;
 ...
 INSERT INTO public.poc_call_events(call_id,event,metadata) VALUES(c.id,'call_finalized',jsonb_build_object(
  ...
  'selectedLoadId',c.selected_load_id,'negotiation',r->'session'->'negotiation','booking',b,'bookingConfirmed',coalesce(b->>'status'='confirmed',false) AND coalesce(b->>'simulated','false')<>'true'));
 RETURN r;
END $$;
